...
要生成虚拟Dom，我们可以使用 **h函数**，就是render函数里面传入的那个**h函数**。它可以接受多种类型的参数，但其实它内部只干了一件事，就是执行`vnode函数`。根据传入**h函数**的参数来决定执行`vnode函数`时传入的参数。那么`vnode函数`又是干什么的呢？`vnode函数`其实也只干了一件事，就是把传入**h函数**的参数转化为一个对象，即虚拟Dom。
